"""
Data volumes workflow implementation for generic storage devices.

This module contains the vendor-specific implementation of the data volumes workflow
for generic storage devices.
"""
...
def execute_data_volumes_workflow(ctx: typer.Context, default_protocol: StorageProtocol) -> Optional[str]:
    """
    Execute a data volumes workflow for generic storage devices and return the generated user data.

    This function leverages the existing generic storage device workflow by calling it with a secondary context configured for data volumes.
    ...
    try:
        # Import the generic iSCSI and NVMe workflow functions
        ...
        # Call the existing generic iSCSI or NVMe workflow with data volumes context
        # This will automatically handle all the generic-specific logic without duplication
