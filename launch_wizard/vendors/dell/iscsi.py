...
from launch_wizard.vendors.dell.iscsi_utils import dell_create_iscsi_host, dell_get_iscsi_target_endpoints_and_iqns
...
def iscsi(
    ...
    """
    Configure and launch an EC2 instance with Dell PowerStore iSCSI storage connectivity.

    This command configures Dell PowerStore storage arrays for iSCSI connectivity and launches
    ...
    Console().print(f"Starting the Dell iSCSI workflow for {style_var(feature_name.value)}...")
    ...
    host_id = dell_create_iscsi_host(powerstore_connection, host_name, initiator_iqn, guest_os_type)
    if host_group_name:
    ...
    targets = dell_get_iscsi_target_endpoints_and_iqns(powerstore_connection, target_endpoints)
