...
from launch_wizard.vendors.hpe.iscsi_utils import hpe_create_iscsi_host, hpe_get_iscsi_target_endpoints_and_iqns
...
def iscsi(
    ...
    """
    Configure and launch an EC2 instance with HPE iSCSI storage connectivity.

    This command configures HPE storage arrays for iSCSI connectivity and launches
    an EC2 instance with the appropriate configuration. It handles host creation, volume mapping,
    target discovery, and authentication setup for seamless storage integration.
    ...
    Console().print(f"Starting the HPE iSCSI workflow for {style_var(feature_name.value)}...")
    ...
    host_name = hpe_get_host_name(hpe_client, host_name, StorageProtocol.ISCSI)
    ...
    hpe_create_iscsi_host(hpe_client, host_name, initiator_iqn, guest_os_type)
    if host_set_name:
    ...
    targets = hpe_get_iscsi_target_endpoints_and_iqns(hpe_client, target_endpoints)
