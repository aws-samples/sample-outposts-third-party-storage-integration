...
def validate_ami(ec2_client: boto3.client, ami_id: Optional[str]) -> str:
    ...
    if not ami_id:
        ami_id = prompt_with_trim("Please enter an AMI ID", data_type=str)
...
def validate_root_volume_options(
            ...
            while True:
                try:
                    root_volume_size = prompt_with_trim(
                        f"Please enter the root volume size (minimum {default_volume_size} GiB)", data_type=int
                    )
    ...
    if not root_volume_type:
        ...
        if not auto_confirm(type_prompt, default=True):
            ...
            while True:
                try:
                    root_volume_type = prompt_with_trim("Please enter the root volume type", data_type=EBSVolumeType)
                    break
                except ValueError:
...
def launch_instance(
    ...
    try:
        ...
        if root_volume_device_name:
            ...
            if root_volume_type:
                instance_params["BlockDeviceMappings"][0]["Ebs"]["VolumeType"] = root_volume_type.value
